...
Toasts (the short-lived status messages raised through `INotifier`) have changed in ways that affect both screen reader users and developers. See [Toast Accessibility](#toast-accessibility) for the new setting.

- **Screen reader announcements.** In **TheAdmin**, **TheTheme** and the fallback `Message` shape used by themes without their own `Message` view (such as **TheBlogTheme** and **TheAgencyTheme**), Success and Information toasts now render `role="status"` and `aria-live="polite"` instead of `role="alert"` and `aria-live="assertive"`. Screen readers now announce them after the current speech instead of interrupting it. Warning and Error toasts keep `role="alert"` and `aria-live="assertive"`. If you override `Message.cshtml` in your own theme, consider applying the same severity-to-role mapping.
- **Localized close button.** The toast close button's `aria-label` is now localized through `T["Close"]` instead of being hard-coded in English.
- **`Notifier` constructor.** `Notifier` now takes `IOptions<ToastOptions>` in addition to `ILogger<Notifier>`. Code that instantiates `Notifier` directly, for example in tests, must pass the options, such as `Options.Create(new ToastOptions())`.
- **Custom `INotifier` implementations.** `SuccessAsync(message)` no longer passes `DismissalMilliseconds = 5000`. Custom `INotifier` implementations now receive a `NotifyContext` whose `DismissalMilliseconds` is `null`, and the built-in `Notifier` resolves the default from the configured success dismissal delay. If your implementation relied on receiving `5000`, apply your own default. Explicit durations passed to `SuccessAsync(message, milliseconds)`, `InformationAsync(message, milliseconds)`, `WarningAsync(message, milliseconds)` and `ErrorAsync(message, milliseconds)` are still passed through unchanged.
...
The **General** site settings now have an **Accessibility** tab, placed directly after **Site**, with a **Success message dismissal delay (seconds)** setting. It requires the `Manage general settings` permission.

- Valid values are `0`, or a whole number from `5` to `60`. When the setting has never been saved, the delay is `5` seconds, which matches the previous behavior.
- `0` turns off auto-dismiss, so success toasts stay until the user closes them. This helps users who need more time to read or reach a message.
...
- An explicit duration passed in code, such as `SuccessAsync(message, milliseconds)`, still takes precedence over the setting.

The value is stored in the site settings as `ToastSettings.SuccessDismissalDelaySeconds` and can be exported and imported with the **Accessibility settings** deployment step. There is no `appsettings.json` equivalent. For details, see the [Settings documentation](../reference/modules/Settings/README.md#accessibility).

This release also changes how toasts are announced to screen readers and the `Notifier` constructor. See [Toasts](#toasts) under Breaking Changes.
...
### Background Tasks Bulk Actions

The **Background Tasks** admin list (`/Admin/BackgroundTasks`) now supports bulk actions, similar to the **Features** admin list. Each task has a checkbox, and once more than one task is selected an **Actions** menu appears with **Enable** and **Disable** options that apply to every selected task at once. The list uses the shared `list-management` script, so the per-page **Select All** checkbox and selection count behave consistently with the other admin lists.

### Features Bulk Actions

The **Toggle** bulk action was removed from the **Features** admin list, leaving **Enable** and **Disable**, for consistency with the other admin lists that offer bulk actions. The `Toggle` value was also removed from the `FeaturesBulkAction` enum and its handling in `FeatureService.EnableOrDisableFeaturesAsync`.
